const FAVICON_BLOCK_START = "<!-- NORALOS_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- NORALOS_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- NORALOS_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- NORALOS_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.NORALOS_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.NORALOS_WORKTREE_NAME) ?? nonEmpty(env.NORALOS_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.NORALOS_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
