...
import type { BindMode, DeploymentExposure, DeploymentMode } from "@noralos/shared";
...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.NORALOS_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.NORALOS_AGENT_JWT_SECRET === "string" ? parsed.NORALOS_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  return {
    status: "warn",
    message: "missing (run `pnpm noralos onboard`)",
  };
}
...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const envFilePath = resolveNoralosEnvPath();
