...
<head>
    <!-- NORALOS_RUNTIME_BRANDING_START -->
    <!-- NORALOS_RUNTIME_BRANDING_END -->
    <!-- NORALOS_FAVICON_START -->
...
describe("ui branding", () => {
  it("detects worktree mode from NORALOS_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ NORALOS_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ NORALOS_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ NORALOS_IN_WORKTREE: "false" })).toBe(false);
  });

  it("resolves name, color, and text color for worktree branding", () => {
    const branding = getWorktreeUiBranding({
      NORALOS_IN_WORKTREE: "true",
      NORALOS_WORKTREE_NAME: "paperclip-pr-432",
      NORALOS_WORKTREE_COLOR: "#4f86f7",
    });
