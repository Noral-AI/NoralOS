...
describe("agent instructions service", () => {
  const originalPaperclipHome = process.env.NORALOS_HOME;
  const originalPaperclipInstanceId = process.env.NORALOS_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalPaperclipHome === undefined) delete process.env.NORALOS_HOME;
    else process.env.NORALOS_HOME = originalPaperclipHome;
    if (originalPaperclipInstanceId === undefined) delete process.env.NORALOS_INSTANCE_ID;
    else process.env.NORALOS_INSTANCE_ID = originalPaperclipInstanceId;
