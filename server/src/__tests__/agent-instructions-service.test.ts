...
describe("agent instructions service", () => {
  const originalNoralosHome = process.env.NORALOS_HOME;
  const originalNoralosInstanceId = process.env.NORALOS_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalNoralosHome === undefined) delete process.env.NORALOS_HOME;
    else process.env.NORALOS_HOME = originalNoralosHome;
    if (originalNoralosInstanceId === undefined) delete process.env.NORALOS_INSTANCE_ID;
    else process.env.NORALOS_INSTANCE_ID = originalNoralosInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const noralosHome = await makeTempDir("paperclip-agent-instructions-home-");
    const externalRoot = await makeTempDir("paperclip-agent-instructions-external-");
    cleanupDirs.add(noralosHome);
    cleanupDirs.add(externalRoot);
    process.env.NORALOS_HOME = noralosHome;
    process.env.NORALOS_INSTANCE_ID = "test-instance";
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const noralosHome = await makeTempDir("paperclip-agent-instructions-recover-");
    cleanupDirs.add(noralosHome);
    process.env.NORALOS_HOME = noralosHome;
    process.env.NORALOS_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const noralosHome = await makeTempDir("paperclip-agent-instructions-stale-managed-");
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const noralosHome = await makeTempDir("paperclip-agent-instructions-heal-write-");
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const noralosHome = await makeTempDir("paperclip-agent-instructions-heal-delete-");
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const noralosHome = await makeTempDir("paperclip-agent-instructions-partial-managed-");
