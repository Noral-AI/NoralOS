...
const isSameFile = existsSync(CWD_ENV_PATH) && existsSync(NORALOS_ENV_FILE_PATH)
  ? realpathSync(CWD_ENV_PATH) === realpathSync(NORALOS_ENV_FILE_PATH)
  : CWD_ENV_PATH === NORALOS_ENV_FILE_PATH;
...
function detectTailnetBindHost(): string | undefined {
  const explicit = process.env.NORALOS_TAILNET_BIND_HOST?.trim();
...
export function loadConfig(): Config {
  ...
  const storageProviderFromEnvRaw = process.env.NORALOS_STORAGE_PROVIDER;
  ...
  const storageS3Bucket = process.env.NORALOS_STORAGE_S3_BUCKET ?? fileStorage?.s3?.bucket ?? "paperclip";
  const storageS3Region = process.env.NORALOS_STORAGE_S3_REGION ?? fileStorage?.s3?.region ?? "us-east-1";
  const storageS3Endpoint = process.env.NORALOS_STORAGE_S3_ENDPOINT ?? fileStorage?.s3?.endpoint ?? undefined;
  const storageS3Prefix = process.env.NORALOS_STORAGE_S3_PREFIX ?? fileStorage?.s3?.prefix ?? "";
  const storageS3ForcePathStyle =
    process.env.NORALOS_STORAGE_S3_FORCE_PATH_STYLE !== undefined
      ? process.env.NORALOS_STORAGE_S3_FORCE_PATH_STYLE === "true"
      : (fileStorage?.s3?.forcePathStyle ?? false);
  const feedbackExportBackendUrl =
    process.env.NORALOS_FEEDBACK_EXPORT_BACKEND_URL?.trim() ||
    process.env.NORALOS_TELEMETRY_BACKEND_URL?.trim() ||
    undefined;
  const feedbackExportBackendToken =
    process.env.NORALOS_FEEDBACK_EXPORT_BACKEND_TOKEN?.trim() ||
    process.env.NORALOS_TELEMETRY_BACKEND_TOKEN?.trim() ||
    undefined;

  const deploymentModeFromEnvRaw = process.env.NORALOS_DEPLOYMENT_MODE;
  ...
  const bindFromEnvRaw = process.env.NORALOS_BIND;
  const bindFromEnv =
  ...
  const customBindHost = process.env.NORALOS_BIND_HOST ?? fileConfig?.server.customBindHost;
  const authBaseUrlModeFromEnvRaw = process.env.NORALOS_AUTH_BASE_URL_MODE;
  const authBaseUrlModeFromEnv =
    authBaseUrlModeFromEnvRaw &&
    AUTH_BASE_URL_MODES.includes(authBaseUrlModeFromEnvRaw as AuthBaseUrlMode)
      ? (authBaseUrlModeFromEnvRaw as AuthBaseUrlMode)
      : null;
  const publicUrlFromEnv = process.env.NORALOS_PUBLIC_URL;
  ...
  const disableSignUpFromEnv = process.env.NORALOS_AUTH_DISABLE_SIGN_UP;
  const authDisableSignUp: boolean =
    disableSignUpFromEnv !== undefined
      ? disableSignUpFromEnv === "true"
      : (fileConfig?.auth?.disableSignUp ?? false);
  const allowedHostnamesFromEnvRaw = process.env.NORALOS_ALLOWED_HOSTNAMES;
  ...
  const companyDeletionEnvRaw = process.env.NORALOS_ENABLE_COMPANY_DELETION;
  const companyDeletionEnabled =
    companyDeletionEnvRaw !== undefined
      ? companyDeletionEnvRaw === "true"
      : deploymentMode === "local_trusted";
  const databaseBackupEnabled =
    process.env.NORALOS_DB_BACKUP_ENABLED !== undefined
      ? process.env.NORALOS_DB_BACKUP_ENABLED === "true"
      : (fileDatabaseBackup?.enabled ?? true);
  const databaseBackupIntervalMinutes = Math.max(
    1,
    Number(process.env.NORALOS_DB_BACKUP_INTERVAL_MINUTES) ||
      fileDatabaseBackup?.intervalMinutes ||
      60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    Number(process.env.NORALOS_DB_BACKUP_RETENTION_DAYS) ||
      fileDatabaseBackup?.retentionDays ||
      7,
  );
  const databaseBackupDir = resolveHomeAwarePath(
    process.env.NORALOS_DB_BACKUP_DIR ??
  ...
  return {
    ...
    uiDevMiddleware: process.env.NORALOS_UI_DEV_MIDDLEWARE === "true",
    secretsProvider,
    secretsStrictMode,
