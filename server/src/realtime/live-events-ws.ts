...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@noralos/db";
...
interface IncomingMessageWithContext extends IncomingMessage {
  noralosUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).noralosUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.noralosUpgradeContext = context;
