 ...
 */
import { pluginManifestV1Schema } from "@noralos/shared";
import type { NoralosPluginManifestV1 } from "@noralos/shared";
import { PLUGIN_API_VERSION } from "@noralos/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): NoralosPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as NoralosPluginManifestV1,
        };
      }
