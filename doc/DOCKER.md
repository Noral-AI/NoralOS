...
Run NoralOS in Docker without installing Node or pnpm locally.
...
```sh
NORALOS_PORT=3200 NORALOS_DATA_DIR=../data/pc \
  docker compose -f docker/docker-compose.quickstart.yml up --build
```

**Note:** `NORALOS_DATA_DIR` is resolved relative to the compose file (`docker/`), so `../data/pc` maps to `data/pc` in the project root.

If you change host port or use a non-local domain, set `NORALOS_PUBLIC_URL` to the external URL you will use in browser/auth flows.

Pass `OPENAI_API_KEY` and/or `ANTHROPIC_API_KEY` to enable local adapter runs.

### Full stack (with PostgreSQL)

NoralOS server + PostgreSQL 17. The database is health-checked before the server starts.
...
For authenticated deployments, set one canonical public URL and let NoralOS derive auth/callback defaults:
...
services:
  paperclip:
    environment:
      ...
      NORALOS_PUBLIC_URL: https://desk.koker.net
```

`NORALOS_PUBLIC_URL` is used as the primary source for:
...
Granular overrides remain available if needed (`NORALOS_AUTH_PUBLIC_BASE_URL`, `BETTER_AUTH_URL`, `BETTER_AUTH_TRUSTED_ORIGINS`, `NORALOS_ALLOWED_HOSTNAMES`).

Set `NORALOS_ALLOWED_HOSTNAMES` explicitly only when you need additional hostnames beyond the public URL host (for example Tailscale/LAN aliases or multiple private hostnames).
...
- Adapter environment checks in NoralOS will surface missing auth/CLI prerequisites.

## Podman Quadlet (systemd)

The `docker/quadlet/` directory contains unit files to run NoralOS + PostgreSQL as systemd services via Podman Quadlet.
...
- `npx noralos onboard --yes` completes
...
```sh
HOST_PORT=3200 NORALOS_CLI_VERSION=latest ./scripts/docker-onboard-smoke.sh
NORALOS_DEPLOYMENT_MODE=authenticated NORALOS_DEPLOYMENT_EXPOSURE=private ./scripts/docker-onboard-smoke.sh
SMOKE_DETACH=true SMOKE_METADATA_FILE=/tmp/noralos-smoke.env NORALOS_CLI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```
...
- Smoke script defaults host port to `3131` to avoid conflicts with local NoralOS on `3100`.
- Smoke script also defaults `NORALOS_PUBLIC_URL` to `http://localhost:<HOST_PORT>` so bootstrap invite URLs and auth callbacks use the reachable host port instead of the container's internal `3100`.
