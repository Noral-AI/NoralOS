...
NoralOS uses PostgreSQL via [Drizzle ORM](https://orm.drizzle.team/). There are three ways to run the database, from simplest to most production-ready.
...
1. Creates a `~/.noralos/instances/default/db/` directory for storage
2. Ensures the `noralos` database exists
3. Runs migrations automatically for empty databases
4. Starts serving requests

Data persists across restarts in `~/.noralos/instances/default/db/`. To reset local dev data, delete that directory.
...
When `DATABASE_URL` is unset, this command targets the current embedded PostgreSQL instance for your active NoralOS config/instance.
...
Docker note: the Docker quickstart image also uses embedded PostgreSQL by default. Persist `/noralos` to keep DB state across container restarts (see `doc/DOCKER.md`).
...
# DATABASE_URL=postgres://noralos:paperclip@localhost:5432/noralos
```
...
If you later run the app with a pooled runtime URL, set `DATABASE_MIGRATION_URL` to the direct connection URL. NoralOS uses it for startup schema checks/migrations and plugin namespace migrations, while the app continues to use `DATABASE_URL` for runtime queries:
...
If your hosted database requires transaction-pooling-only connections, use a direct or session-pooled connection for NoralOS until runtime pooling support is documented in this guide. Do not edit database client source files as part of deployment setup.
...
| `DATABASE_URL` | Mode |
|---|---|
| Not set | Embedded PostgreSQL (`~/.noralos/instances/default/db/`) |
...
NoralOS stores secret metadata and versions in:
...
- `NORALOS_SECRETS_MASTER_KEY` (32-byte key as base64, hex, or raw 32-char string)
- `NORALOS_SECRETS_MASTER_KEY_FILE` (custom key file path)
