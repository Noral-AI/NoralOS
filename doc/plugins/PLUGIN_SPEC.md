# NoralOS Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for NoralOS's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main NoralOS app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary NoralOS HTTP APIs directly.
...
NoralOS plugin design is based on the following assumptions:

1. NoralOS is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core NoralOS business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by NoralOS core.
...
2. Let plugins add major capabilities without editing NoralOS core.
...
4. Depend on project-local plugin folders such as `.noralos/plugins`.
...
The single NoralOS deployment an operator installs and controls.

### 5.2 Company

A first-class NoralOS business object inside the instance.
...
A trusted in-process extension loaded directly by NoralOS core.
...
An installable instance-wide extension package loaded through the NoralOS plugin runtime.
...
NoralOS has two extension classes.
...
NoralOS already has a concrete workspace model:
...
Plugins live under the NoralOS instance directory.

Suggested layout:

- `~/.noralos/instances/default/plugins/package.json`
- `~/.noralos/instances/default/plugins/node_modules/`
- `~/.noralos/instances/default/plugins/.cache/`
- `~/.noralos/instances/default/data/plugins/<plugin-id>/`
...
NoralOS should add CLI commands:

- `pnpm noralos plugin list`
- `pnpm noralos plugin install <package[@version]>`
- `pnpm noralos plugin uninstall <plugin-id>`
- `pnpm noralos plugin upgrade <plugin-id> [version]`
- `pnpm noralos plugin doctor <plugin-id>`
...
Plugins may contribute tools that NoralOS agents can use during runs.
...
- NoralOS server starts one worker process per installed plugin
...
Receives one typed NoralOS domain event.
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@noralos/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@noralos/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@noralos/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@noralos/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@noralos/plugin-sdk/ui`
...
The `@noralos/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the NoralOS secret provider system rather than a plain text input
...
1. Core NoralOS data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing NoralOS objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual NoralOS product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if NoralOS later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing NoralOS secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm noralos plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the NoralOS server. This is a normative requirement, not optional.
...
### 27.1 `@noralos/plugin-test-harness`
...
```ts
import { createTestHarness } from "@noralos/plugin-test-harness";
...
For developing a plugin against a running NoralOS instance:

- The operator installs the plugin from a local path: `pnpm noralos plugin install ./path/to/plugin`
...
- `@noralos/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@noralos/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@noralos/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@noralos/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@noralos/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@noralos/plugin-sdk/ui`
...
NoralOS should not implement a generic in-process hook bag modeled directly after local coding tools.

NoralOS should implement:
...
That is the complete target design for the NoralOS plugin system.
