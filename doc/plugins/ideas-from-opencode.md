...
NoralOS V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a NoralOS-shaped design.

Assumption for this document: NoralOS is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class NoralOS objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into NoralOS.

The main conclusion is:

- NoralOS should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- NoralOS should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- NoralOS should use multiple extension classes instead of one generic plugin bag:
...
If NoralOS does this well, the examples you listed become straightforward:
...
Relevant NoralOS files reviewed for current extension seams:
...
It is too dangerous for NoralOS core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for NoralOS — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
NoralOS should absolutely do this.
...
NoralOS should copy this discipline.
...
NoralOS should adopt this style for plugin actions, automations, and UI schemas.
...
NoralOS can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for NoralOS too.

## What NoralOS Should Not Copy Directly
...
NoralOS is a control plane for an operator-managed instance with company objects.
...
NoralOS is not project-scoped. It is instance-scoped.
...
For NoralOS, equivalent hooks into:
...
NoralOS should not allow plugins to silently replace:
...
For NoralOS it would be risky because it combines:
...
NoralOS should require an explicit operator install step.

## Why NoralOS Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | NoralOS |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted NoralOS instance |
...
That means NoralOS should borrow the good ideas from `opencode` but use a stricter architecture.

## NoralOS Already Has Useful Pre-Plugin Seams

NoralOS has several extension-like seams already:
...
This is good news.
NoralOS does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended NoralOS Plugin Model
...
NoralOS already has this pattern implicitly:
...
- `plugin` means integration code that talks to NoralOS through a typed plugin protocol and capability model
...
- invoke explicit NoralOS actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@noralos/plugin-sdk/ui`) with design tokens and common components.
...
NoralOS should treat plugin installation as a global instance-level action.

Examples:

- install `@paperclip/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over NoralOS objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

NoralOS already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for NoralOS too.
...
Plugin install, uninstall, upgrade, and config changes should take effect without restarting the NoralOS server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. NoralOS's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@noralos/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@noralos/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For NoralOS

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@noralos/plugin-sdk";
...
export default definePlugin({
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit NoralOS entities
    });
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@noralos/plugin-sdk/ui";
...
Any NoralOS plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific NoralOS objects, those are plugin data, not enable/disable boundaries.
...
- reuse core tables when the data is clearly part of NoralOS itself
...
If a concept is becoming part of NoralOS's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of NoralOS itself
...
### 3. Scope plugin data to NoralOS objects before adding custom schemas

A lot of plugin data naturally hangs off existing NoralOS objects:
...
If NoralOS eventually needs extension-owned tables, I would only allow that for:
...
- Board opens a project page and reviews the diff for that project's workspace without leaving NoralOS.
- Board reviews the diff after a run without leaving NoralOS.
...
This plugin syncs NoralOS work with Linear. It is useful for:

- importing backlog from Linear
- linking NoralOS issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between NoralOS and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into NoralOS.
- NoralOS issue status changes push to Linear and Linear comments arrive back through webhooks.
...
This plugin syncs NoralOS issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to NoralOS issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with NoralOS task governance
...
- Plugin settings:
  ...
  - whether new NoralOS issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active NoralOS issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into NoralOS.
- GitHub webhooks update status/comment state in NoralOS.
- A PR is linked back to the NoralOS issue so the board can follow delivery status.
...
This plugin surfaces external metrics and dashboards inside NoralOS. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating NoralOS issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full NoralOS plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the NoralOS dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a NoralOS issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of NoralOS core; keep it as additive signal and issue creation
...
This plugin pulls Stripe revenue and subscription data into NoralOS. It is useful for:
...
- Failed payment spikes or churn events can generate NoralOS issues for follow-up.
...
- Stripe data should stay additive to NoralOS core
...
- plugin UI bundle loading, host bridge, `@noralos/plugin-sdk/ui`
...
- `@noralos/plugin-test-harness` and `create-paperclip-plugin` starter template
...
NoralOS should not implement "an OpenCode-style generic in-process hook system."
NoralOS should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In NoralOS
...
8. Ship `@noralos/plugin-test-harness` and `create-paperclip-plugin` starter template.
