...
For `claude_local` and `codex_local`, NoralOS assumes the CLI is already installed and authenticated on the host machine.
...
NoralOS stores resumable session state per `(agent, taskKey, adapterType)`.
