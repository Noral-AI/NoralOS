...
This spec defines how NoralOS actually runs agents while staying runtime-agnostic.
...
1. NoralOS is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, NoralOS manages lifecycle, and on exit NoralOS parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `paperclip.skill` (shared NoralOS skill text block)
...
2. Preferred transport is env vars (`NORALOS_*`) injected at runtime.
