...
NoralOS supports two runtime modes:
...
NoralOS now treats **bind** as a separate concern from auth:
...
pnpm noralos onboard --yes --bind lan
pnpm noralos run --bind tailnet
```
...
When running `authenticated` mode, if the only instance admin is `local-board`, NoralOS emits a startup warning with a one-time high-entropy claim URL.
