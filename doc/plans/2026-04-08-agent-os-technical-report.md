# Agent OS Technical Report for NoralOS
...
`agent-os` is not a competitor to NoralOS's core product. It is an execution substrate: an embedded, VM-like runtime for agents, tools, filesystems, and session orchestration. NoralOS is a control plane: company scoping, task hierarchy, approvals, budgets, activity logs, workspaces, and governance.

The strongest takeaway is not "copy agent-os wholesale." The strongest takeaway is that NoralOS could selectively use its runtime ideas to improve local agent execution safety, reproducibility, and portability while keeping all company/task/governance logic in NoralOS.

My recommendation is:

1. Do not merge agent-os concepts into the NoralOS core product model.
...
4. Do not import its workflow/cron/queue abstractions into NoralOS core until they are reconciled with NoralOS's issue/comment/governance model.
...
That gives `agent-os` a much stronger isolation story than NoralOS's current "launch a host CLI in a workspace" local adapter approach.
...
This is directly relevant to NoralOS because our adapter layer still normalizes each CLI agent in a fairly bespoke way.

## NoralOS anchor points

The most relevant current NoralOS surfaces for any future `agent-os` integration are:
...
## What NoralOS can learn from it

### 1. A safer local execution substrate

NoralOS's local adapters currently run host CLIs in managed workspaces and rely on adapter-specific behavior plus process-level controls. That is pragmatic, but weakly isolated.
...
This fits NoralOS because it improves execution safety without changing the control-plane model.

### 2. Snapshotted execution roots instead of only mutable workspaces

NoralOS already has strong execution-workspace concepts, but they are repo/worktree-centric. `agent-os` adds a stronger "start from known lower layers, write into a disposable upper layer" model.
...
NoralOS has governance at the company/task level:
...
It has less structure at the runtime capability level. `agent-os` offers a clear vocabulary that NoralOS could adopt even without adopting the runtime itself:
...
NoralOS's plugin system and adapters already have the beginnings of a controlled extension surface. `agent-os` reinforces the value of exposing capabilities as typed tools rather than raw shell access.

Concrete NoralOS uses:
...
This aligns well with NoralOS's governance story.

### 5. Better adapter normalization around sessions and capabilities

NoralOS's adapter contract already supports execution results, session params, environment tests, skill syncing, quota windows, and optional `detectModel()`. But much of the per-agent behavior is still adapter-specific.
...
NoralOS does not need ACP everywhere, but it would benefit from a more formal internal session capability model inspired by this.
...
NoralOS can adopt that philosophy directly:
...
## What does not fit NoralOS well

### 1. Its built-in orchestration primitives overlap the wrong layer

`agent-os` includes cron/session/workflow style primitives inside the runtime package. NoralOS already has higher-level orchestration concepts:
...
If NoralOS copied `agent-os` cron/workflow/queue ideas directly into core, we would likely duplicate orchestration across two layers. That would blur ownership and make debugging harder.

NoralOS should keep orchestration authoritative at the control-plane layer.
...
Those are NoralOS's differentiators. They should not be displaced by runtime abstractions.
...
The permission model is good, but it is low-level. NoralOS would still need to answer:
...
- keep NoralOS heartbeat/task/workspace/budget logic unchanged
...
- NoralOS's Codex adapter is already important and carries repo-specific behavior
...
- NoralOS still records logs, summaries, cost metadata, and issue comments normally
...
Even without using `agent-os`, NoralOS should consider standardizing adapter/runtime permissions around a vocabulary like:
...
NoralOS should evaluate whether some execution workspaces can be backed by:
...
NoralOS plugins and adapters should continue moving toward explicit typed tools over ad hoc shell access. `agent-os` confirms that this is the right direction.
...
### Recommendation E: do not import runtime-level workflows into NoralOS core

NoralOS should not copy `agent-os` cron/workflow/queue concepts into core orchestration yet.
...
- moving NoralOS orchestration into agent-os workflows
...
`agent-os` is useful to NoralOS as an execution technology reference, not as a product model.

NoralOS should treat it the same way it treats sandboxes or agent CLIs:
