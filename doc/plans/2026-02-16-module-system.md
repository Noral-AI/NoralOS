...
NoralOS's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends NoralOS's API, UI, and data model | Yes |
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@noralos/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@noralos/core";
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@noralos/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@noralos/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
...
```bash
pnpm noralos store list                    # browse available modules and templates
pnpm noralos store install <module-id>     # install a module
pnpm noralos store import <template-id>    # import a company template
pnpm noralos store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one NoralOS instance | `server:started` |
...
10. **`@noralos/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
