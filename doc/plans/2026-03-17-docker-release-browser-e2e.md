...
Today release smoke testing for published NoralOS packages is manual and shell-driven:
...
- runs `noralos@${NORALOS_CLI_VERSION}` inside Docker
- waits for health
...
- the suite passes locally against both:
  - `NORALOS_CLI_VERSION=canary`
  - `NORALOS_CLI_VERSION=latest`
...
1. A published `noralos@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `noralos@latest` Docker install can be smoke-tested by Playwright in CI.
