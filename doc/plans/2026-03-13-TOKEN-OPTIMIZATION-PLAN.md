...
- local runtime data from the default NoralOS instance on 2026-03-13
...
3. **Repeated context reacquisition.** The `noralos` skill tells agents to re-fetch assignments, issue details, ancestors, and full comment threads on every heartbeat. The API does not currently offer efficient delta-oriented alternatives.
...
The `noralos` skill currently tells agents to do this on essentially every heartbeat:
...
- NoralOS discovers repo skills from the current checkout, then symlinks them into `$CODEX_HOME/skills` or `~/.codex/skills`.
- If an existing NoralOS skill symlink already points at another live checkout, the current implementation skips it instead of repointing it.
- This can leave Codex using stale skill content from a different worktree even after NoralOS-side skill changes land.
...
- Store both:
  ...
  - NoralOS-normalized per-run usage
...
For `codex_local`, this also requires isolating the Codex skill home per worktree or teaching NoralOS to repoint its own skill symlinks when the source checkout changes. Otherwise prompt and skill improvements in the active worktree may not reach the running agent.
...
Update the `noralos` skill so the default pattern becomes:
...
- For `codex_local`, either:
  ...
  - treat NoralOS-owned Codex skill symlinks as repairable when they point at a different checkout
...
4. heartbeat delta APIs + `noralos` skill rewrite
