...
Turn the `agent-os` research into a low-risk NoralOS execution plan that preserves NoralOS's control-plane model while testing the few runtime ideas that appear worth adopting.

## Decision summary

NoralOS should not absorb `agent-os` as a product model or orchestration layer.

NoralOS should evaluate `agent-os` in three narrow areas:
...
This work is successful when NoralOS has:
...
- a proposed runtime capability model that fits current NoralOS adapters
...
- replace NoralOS heartbeats, issues, comments, approvals, or budgets with `agent-os` primitives
- introduce Rust/sidecar requirements for all local execution paths
- migrate all adapters at once
- add runtime workflow/queue abstractions to NoralOS core

## Existing NoralOS integration points
...
- prove that NoralOS can drive one local agent through an `agent-os` runtime without breaking heartbeat semantics
...
- keep NoralOS's existing heartbeat, issue, workspace, and comment flow authoritative
...
- introduce a NoralOS-native capability vocabulary without coupling the product to `agent-os`
...
- determine whether a layered/snapshotted root model improves some NoralOS workloads
...
- identify where NoralOS should prefer explicit typed tools over ambient shell access
...
- one concrete proposal for typed-tool adoption in NoralOS
...
- runtime experimentation could blur the boundary between NoralOS as control plane and NoralOS as execution platform
