...
- New config/env settings:
  - `NORALOS_AGENT_JWT_SECRET`
  - `NORALOS_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `NORALOS_AGENT_JWT_ISSUER` (default: `noralos`)
  - `NORALOS_AGENT_JWT_AUDIENCE` (default: `paperclip-api`)
...
1. In:
   ...
   inject `NORALOS_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `NORALOS_API_KEY`, do not overwrite it.
- Continue injecting:
  - `NORALOS_AGENT_ID`
  - `NORALOS_COMPANY_ID`
  - `NORALOS_API_URL`
...
- Local adapters authenticate without manual `NORALOS_API_KEY` config.
