...
`PAP-447` asks how NoralOS should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same NoralOS instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into NoralOS:

- some operators use NoralOS to manage NoralOS and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as NoralOS
- Claude and Codex expose different built-in affordances, so NoralOS should not overfit to one tool

## Core Product Decision

NoralOS should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by NoralOS
...
- NoralOS should not assume that every adapter can see or use a host filesystem path directly
...
If NoralOS makes worktrees agent-first, it will blur:
...
- NoralOS core resolves and records execution workspace state
...
That gives NoralOS room to support:
...
- NoralOS handles workspace runtime services under the hood using project/workspace policy
...
Once NoralOS is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that NoralOS cannot infer
...
- when issue moves to done, should NoralOS auto-commit?
- should it auto-push?
...
NoralOS already has the right foundation for a project-first model.
...
For server-spawned local adapters, NoralOS already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that NoralOS should mirror directly

Implication:

- for `codex_local`, NoralOS should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but NoralOS should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, NoralOS should separate:
...
NoralOS should be able to express intentions such as:
...
These are not specific to NoralOS. They are a common property of working in a dev workspace, whether local or remote.
...
Do not model this as a NoralOS-specific `devServer` flag.
...
NoralOS should distinguish between:
...
NoralOS should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, NoralOS should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to NoralOS
...
For runtime services specifically, NoralOS should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as NoralOS-only dev-server flags
...
- same embedded DB / NoralOS instance can be reused through env/config injection when appropriate
...
- NoralOS can decide whether to reuse or start a fresh service deterministically
...
3. Test against NoralOS-on-NoralOS workflows first.
...
7. Workspace runtime services are modeled generically, not as NoralOS-specific dev-server toggles.
