...
- Team of 3 can use one shared NoralOS instance.
...
NoralOS should manage the **issue → workspace → preview/PR → review handoff** lifecycle, but leave diffs/review/merge to external tools.
...
execution:
  ...
  preview:
    command: "pnpm dev --port $NORALOS_PREVIEW_PORT"
    healthPath: "/"
    ttlMinutes: 120
  vcs:
    provider: github
...
- A chat plugin can converse without requiring chat to become the core NoralOS product.
...
- **Keep** NoralOS as the board-level control plane.
