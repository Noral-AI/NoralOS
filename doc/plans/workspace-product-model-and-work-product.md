...
NoralOS needs to support two very different but equally valid ways of working:
...
Today, NoralOS already has the beginnings of this model:
...
- making NoralOS feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing NoralOS users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted NoralOS deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning NoralOS into a full code review product
...
This object must be recorded explicitly so that NoralOS can:
...
NoralOS should treat PRs as a type of work product linked back to:
...
NoralOS already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted NoralOS must be a first-class deployment mode

NoralOS cannot assume that it is running on the same machine as the code.

In cloud deployments, NoralOS may:
...
- `Runtime service`: a process or service NoralOS owns or tracks for a workspace
...
It also lets cloud-hosted NoralOS deployments point at codebases and remotes without pretending the NoralOS host has direct filesystem access.
...
Without an explicit `execution workspace` record, NoralOS has nowhere stable to attach:
...
NoralOS needs a single place to show:
...
- NoralOS-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without NoralOS owning the execution host
...
## 1. NoralOS host and execution host must be decoupled
...
without the NoralOS host itself running local git or local preview processes.
...
NoralOS should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for NoralOS.
...
NoralOS should keep workspace automation centered on a small set of hooks:
...
NoralOS should provide the equivalent visibility in the project `Code` surface:
...
NoralOS should treat execution workspaces as the natural home for:
...
NoralOS should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
NoralOS should continue treating cleanup policy as part of the core workspace model:
...
NoralOS should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the NoralOS host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

NoralOS should borrow Worktrunk's good execution patterns while keeping the broader NoralOS model:
...
That keeps NoralOS focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted NoralOS deployments remain first-class

That is a better fit for NoralOS than either extreme:
