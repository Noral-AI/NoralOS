...
NoralOS already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model NoralOS should implement next.

## Product Goals

NoralOS should let operators:
...
For the next implementation pass, NoralOS should enforce these defaults:
...
NoralOS should separate subscription-included usage from billed spend:
...
NoralOS should have two threshold classes:
...
NoralOS must know that a project is budget-paused and enforce it.
...
NoralOS must also block execution before new work starts.
