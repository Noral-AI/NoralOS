...
This document defines the rollout plan for adapter-wide skill support in NoralOS.
...
- NoralOS stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

NoralOS currently has these adapters:
...
1. Desired skills live in NoralOS for every adapter.
...
These adapters have a stable local skills directory that NoralOS can read and manage.
...
These adapters do not have a meaningful NoralOS-owned persistent install state.
...
- desired skills stored in NoralOS
...
- runtime already injects NoralOS skills into `~/.cursor/skills`
...
- runtime already injects NoralOS skills into `~/.gemini/skills`
...
- runtime already injects NoralOS skills into `~/.pi/agent/skills`
...
- NoralOS can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects NoralOS skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- NoralOS may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed NoralOS skills that are clearly marked as NoralOS-managed
...
- NoralOS stores desired skills only
...
- avoid destructive wording unless NoralOS can prove a skill is NoralOS-managed
...
That gets NoralOS from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
