...
This document defines the next-stage plan for NoralOS company import/export.

The core shift is:

- move from a NoralOS-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside NoralOS.
...
NoralOS already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just NoralOS
...
11. adapter-aware skill sync surfaces so NoralOS can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point NoralOS at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a NoralOS sidecar.
...
NoralOS must not redefine `SKILL.md`.
...
### 5.4 Base Package Vs NoralOS Extension
...
- base package:
  ...
  - no NoralOS-only runtime fields by default
- NoralOS extension:
  - `.paperclip.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by NoralOS tools as a sidecar while the base package stays readable
...
NoralOS import/export should support these entity kinds:
...
- NoralOS can treat “this set of agents came from team package X” as the imported-team model
...
NoralOS should support source references in package metadata with:
...
NoralOS should define a new adapter surface area around skills:
...
Planned NoralOS behavior:

- if an adapter supports read, NoralOS should show current skills in the UI
- if an adapter supports write, NoralOS should let the user enable/disable imported skills
- if an adapter supports sync, NoralOS should compute desired vs actual state and offer reconcile actions
...
## 10. Storage Model Inside NoralOS
...
NoralOS should add managed package/provenance records so imports are not anonymous one-off copies.
...
1. Should imported skill packages be stored as managed package files in NoralOS storage, or only referenced at import time?
...
3. Should NoralOS support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps NoralOS aligned with:
