...
Low-level reference for how NoralOS packages are prepared and published to npm.
...
NoralOS no longer uses release branches or Changesets for publishing.

## Why the CLI needs special packaging

The CLI package, `noralos`, imports code from workspace packages such as:
...
### Manual first publish for `@noralos/ui`
...
- If `npm view @noralos/ui version` already returns the same version that is in [`ui/package.json`](../ui/package.json), do not republish. Bump the version or use the normal repo-wide release flow in [`scripts/release.sh`](../scripts/release.sh).
...
- For an organization-scoped package like `@noralos/ui`, the `noralos` npm organization must exist and the publisher must be a member with permission to publish to that scope.
...
NoralOS uses calendar versions:
...
- every published internal `@noralos/*` dependency referenced by that manifest exists on npm
