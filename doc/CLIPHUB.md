...
ClipHub is the public registry where people share, discover, and download NoralOS company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to NoralOS what a package registry is to a programming language. NoralOS already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in NoralOS — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own NoralOS instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the NoralOS spec. It contains:
...
```
Downloads the template and creates a new company in your local NoralOS instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within NoralOS, export your company as a template, then publish:
...
ClipHub is a **separate service** from NoralOS itself. NoralOS is self-hosted; ClipHub is a hosted registry that NoralOS instances talk to.
...
| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with NoralOS) |
| Backend | TypeScript + Hono (consistent with NoralOS) |
| Database | PostgreSQL |
...
5. NoralOS creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in NoralOS until it works well
...
## Relationship to NoralOS

ClipHub is **not required** to use NoralOS. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to NoralOS what a package registry is to a language runtime: optional, but transformative.
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with NoralOS's own philosophy)
