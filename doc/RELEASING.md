...
Maintainer runbook for shipping NoralOS across npm, GitHub, and the website-facing changelog surface.
...
NoralOS uses calendar versions that still fit semver syntax:
...
2. **npm** — `noralos` and public workspace packages are published
...
```bash
npx noralos@canary onboard
# or
npx noralos@canary onboard --data-dir "$(mktemp -d /tmp/noralos-canary.XXXXXX)"
```
...
claude --print --output-format stream-json --verbose --dangerously-skip-permissions --model claude-opus-4-6 "Use the release-changelog skill to draft or update releases/v${VERSION}.md for NoralOS. Read doc/RELEASING.md and .agents/skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
```
...
```bash
NORALOS_CLI_VERSION=canary ./scripts/docker-onboard-smoke.sh
```

For the current stable:

```bash
NORALOS_CLI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

Useful isolated variants:

```bash
HOST_PORT=3232 DATA_DIR=./data/release-smoke-canary NORALOS_CLI_VERSION=canary ./scripts/docker-onboard-smoke.sh
HOST_PORT=3233 DATA_DIR=./data/release-smoke-stable NORALOS_CLI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```
...
- `npx noralos@canary onboard` installs
