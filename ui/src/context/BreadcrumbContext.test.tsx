...
describe("BreadcrumbContext", () => {
  ...
  it("builds page titles with the selected company name before noralAI", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "Anachronist Wiki")).toBe(
      "Inbox • Anachronist Wiki • noralAI",
    );
    expect(
      buildDocumentTitle(
    ...
    ).toBe("PAP-3515 • Issues • Anachronist Wiki • noralAI");
  });

  it("omits blank company names from page titles", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "  ")).toBe("Inbox • noralAI");
    expect(buildDocumentTitle([], null)).toBe("noralAI");
  });
});
