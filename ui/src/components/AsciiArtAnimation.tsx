...
type NoralosSprite = (typeof NORALOS_SPRITES)[number];
...
function spriteSize(sprite: NoralosSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = NORALOS_SPRITES[(baseCol + baseRow) % NORALOS_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = NORALOS_SPRITES[Math.floor(Math.random() * NORALOS_SPRITES.length)]!;
