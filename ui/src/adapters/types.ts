...
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "@noralos/adapter-utils";

export interface StatefulStdoutParser {
  parseLine: (line: string, ts: string) => import("@noralos/adapter-utils").TranscriptEntry[];
  reset: () => void;
}
...
export interface TranscriptParserSource {
  parseStdoutLine: (line: string, ts: string) => import("@noralos/adapter-utils").TranscriptEntry[];
