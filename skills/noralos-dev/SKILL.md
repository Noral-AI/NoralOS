...
| Task | Command |
|------|---------|
| Start server (first time or normal) | `npx noralos run` |
...
| Back up database | `npx noralos db:backup` |
| Health check | `npx noralos doctor --repair` |
| Print env vars | `npx noralos env` |
| Trigger agent heartbeat | `npx noralos heartbeat run --agent-id <id>` |
| Install agent skills locally | `npx noralos agent local-cli <agent> --company-id <id>` |
...
npx noralos worktree:make my-feature --start-point origin/main

# 2. Move into the worktree (path printed by worktree:make) and source the environment
cd <worktree-path>
eval "$(npx noralos worktree env)"
...
npx noralos worktree:merge-history --from paperclip-my-feature --to current --apply

# 6. Clean up
npx noralos worktree:cleanup my-feature
```
...
Before pushing or creating a PR, list remotes and check for one that points at a non-`noralos` GitHub fork:
...
1. **CLI is the only interface to worktrees and databases.** All worktree and database operations MUST go through `npx noralos` / `pnpm noralos` commands. You MUST NOT:
...
2. **If a CLI command fails, stop and report.** Do NOT attempt workarounds. If `worktree:make`, `worktree reseed`, `worktree init`, `worktree:cleanup`, or any other `noralos` command fails:
   - Report the exact error message in your task comment
   - Set the task to `blocked`
   - Suggest running `npx noralos doctor --repair` or recreating the worktree from scratch
...
4. **Starting a dev server in a worktree requires setup first.** The correct sequence is:
   ...
   eval "$(npx noralos worktree env)"
   pnpm install && pnpm build
   npx noralos run          # or pnpm dev

   # If the worktree needs a fresh database:
   npx noralos worktree reseed --seed-mode full

   # If the worktree is broken beyond repair:
   npx noralos worktree:cleanup <name>
   npx noralos worktree:make <name> --seed-mode full
   ```
...
cd <worktree-path>
eval "$(npx noralos worktree env)"   # skip if using the primary instance
...
| Mistake | Fix |
|---------|-----|
| Server won't start | Run `npx noralos doctor --repair` to diagnose and auto-fix |
| Forgetting to source worktree env | Run `eval "$(npx noralos worktree env)"` after cd-ing into the worktree |
