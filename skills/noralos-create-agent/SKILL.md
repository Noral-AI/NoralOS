...
description: >
  Create new agents in NoralOS with governance-aware hiring. Use when you need
  to inspect adapter configuration options, compare existing agent configs,
  draft a new agent prompt/config, and submit a hire request.
---

# NoralOS Create Agent Skill
...
### 2. Discover adapter configuration for this NoralOS instance

```sh
curl -sS "$NORALOS_API_URL/llms/agent-configuration.txt" \
  -H "Authorization: Bearer $NORALOS_API_KEY"

# Then the specific adapter you plan to use, e.g. claude_local:
curl -sS "$NORALOS_API_URL/llms/agent-configuration/claude_local.txt" \
  -H "Authorization: Bearer $NORALOS_API_KEY"
```
...
```sh
curl -sS "$NORALOS_API_URL/api/companies/$NORALOS_COMPANY_ID/agent-configurations" \
  -H "Authorization: Bearer $NORALOS_API_KEY"
```
...
`skills/noralos-create-agent/references/agent-instruction-templates.md`

Generic fallback for no-template hires:
`skills/noralos-create-agent/references/baseline-role-guide.md`
...
```sh
curl -sS "$NORALOS_API_URL/llms/agent-icons.txt" \
  -H "Authorization: Bearer $NORALOS_API_KEY"
```
...
`skills/noralos-create-agent/references/draft-review-checklist.md`

### 8. Submit hire request

```sh
curl -sS -X POST "$NORALOS_API_URL/api/companies/$NORALOS_COMPANY_ID/agent-hires" \
  -H "Authorization: Bearer $NORALOS_API_KEY" \
...
- when the board approves, you will be woken with `NORALOS_APPROVAL_ID`; read linked issues and close/comment follow-up

```sh
curl -sS "$NORALOS_API_URL/api/approvals/<approval-id>" \
  -H "Authorization: Bearer $NORALOS_API_KEY"

curl -sS -X POST "$NORALOS_API_URL/api/approvals/<approval-id>/comments" \
  -H "Authorization: Bearer $NORALOS_API_KEY" \
  -H "Content-Type: application/json" \
...
```sh
curl -sS -X POST "$NORALOS_API_URL/api/issues/<issue-id>/approvals" \
  -H "Authorization: Bearer $NORALOS_API_KEY" \
  -H "Content-Type: application/json" \
...
```sh
curl -sS "$NORALOS_API_URL/api/approvals/$NORALOS_APPROVAL_ID" \
  -H "Authorization: Bearer $NORALOS_API_KEY"

curl -sS "$NORALOS_API_URL/api/approvals/$NORALOS_APPROVAL_ID/issues" \
  -H "Authorization: Bearer $NORALOS_API_KEY"
```
...
- Template index and how to apply a template: `skills/noralos-create-agent/references/agent-instruction-templates.md`
- Individual role templates: `skills/noralos-create-agent/references/agents/`
- Generic baseline role guide (no-template fallback): `skills/noralos-create-agent/references/baseline-role-guide.md`
- Pre-submit draft-review checklist: `skills/noralos-create-agent/references/draft-review-checklist.md`
- Endpoint payload shapes and full examples: `skills/noralos-create-agent/references/api-reference.md`
