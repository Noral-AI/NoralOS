...
base_cwd="${NORALOS_WORKSPACE_BASE_CWD:?NORALOS_WORKSPACE_BASE_CWD is required}"
worktree_cwd="${NORALOS_WORKSPACE_CWD:?NORALOS_WORKSPACE_CWD is required}"
paperclip_home="${NORALOS_HOME:-$HOME/.paperclip}"
paperclip_instance_id="${NORALOS_INSTANCE_ID:-default}"
paperclip_dir="$worktree_cwd/.paperclip"
worktree_config_path="$paperclip_dir/config.json"
worktree_env_path="$paperclip_dir/.env"
worktree_name="${NORALOS_WORKSPACE_BRANCH:-$(basename "$worktree_cwd")}"
...
run_isolated_worktree_init() {
  ...
  if command -v pnpm >/dev/null 2>&1 && pnpm noralos --help >/dev/null 2>&1; then
    (
      cd "$worktree_cwd"
      pnpm noralos worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    )
    return 0
  fi
...
write_fallback_worktree_config() {
  ...
  NORALOS_DIR="$paperclip_dir" \
  SOURCE_CONFIG_PATH="$source_config_path" \
  SOURCE_ENV_PATH="$source_env_path" \
  NORALOS_WORKTREES_DIR="${NORALOS_WORKTREES_DIR:-}" \
  node <<'EOF'
...
async function main() {
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.NORALOS_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.NORALOS_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.NORALOS_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "NORALOS_HOME=" + JSON.stringify(worktreeHome),
    "NORALOS_INSTANCE_ID=" + JSON.stringify(instanceId),
    "NORALOS_CONFIG=" + JSON.stringify(configPath),
    "NORALOS_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "NORALOS_IN_WORKTREE=true",
    "NORALOS_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.NORALOS_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("NORALOS_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
...
if [[ -e "$worktree_config_path" && -e "$worktree_env_path" ]]; then
  echo "Reusing existing isolated NoralOS worktree config at $worktree_config_path" >&2
else
