...
test("parseArgs recognizes publish and skip-build flags", () => {
  assert.deepEqual(parseArgs(["@noralos/adapter-acpx-local", "--publish", "--skip-build"]), {
    help: false,
    selector: "@noralos/adapter-acpx-local",
    publish: true,
    skipBuild: true,
    otp: null,
  });
...
test("resolveTargetPackage matches by package name or dir", () => {
  const packages = [
    { dir: "packages/a", name: "@noralos/a", pkg: {} },
    { dir: "packages/b", name: "@noralos/b", pkg: {} },
  ];

  assert.equal(resolveTargetPackage("@noralos/a", packages).dir, "packages/a");
  assert.equal(resolveTargetPackage("./packages/b", packages).name, "@noralos/b");
});

test("resolveTargetPackage includes the workspace diff plugin bootstrap package", () => {
  const pkg = resolveTargetPackage("@noralos/plugin-workspace-diff");
