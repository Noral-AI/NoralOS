...
# When NORALOS_RELEASE_REUSE_UI_DIST=1 and ui/dist already exists, reuse that
...
case "${NORALOS_RELEASE_REUSE_UI_DIST:-}" in
  1|true|TRUE|yes|YES)
...
if [ "$should_reuse_existing_ui_dist" = true ] && [ -f "$UI_DIST/index.html" ]; then
  echo "  -> Reusing existing @noralos/ui dist output"
else
