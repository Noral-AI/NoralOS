...
Environment overrides:
  ...
  PC_DATA=/tmp/data           NoralOS data dir to use
  NORALOS_HOST=127.0.0.1    Host passed to the onboarded server
  NORALOS_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/noralos-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/noralos-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export NORALOS_HOST="${NORALOS_HOST:-127.0.0.1}"
export NORALOS_PORT="${NORALOS_PORT:-3100}"
export NORALOS_OPEN_ON_LISTEN="${NORALOS_OPEN_ON_LISTEN:-false}"
...
echo "NORALOS_HOST: $NORALOS_HOST"
echo "NORALOS_PORT: $NORALOS_PORT"
