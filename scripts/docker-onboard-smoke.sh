...
NORALOS_CLI_VERSION="${NORALOS_CLI_VERSION:-latest}"
...
NORALOS_DEPLOYMENT_MODE="${NORALOS_DEPLOYMENT_MODE:-authenticated}"
NORALOS_DEPLOYMENT_EXPOSURE="${NORALOS_DEPLOYMENT_EXPOSURE:-private}"
NORALOS_PUBLIC_URL="${NORALOS_PUBLIC_URL:-http://localhost:${HOST_PORT}}"
...
write_metadata_file() {
  ...
  mkdir -p "$(dirname "$SMOKE_METADATA_FILE")"
  {
    ...
    printf 'SMOKE_NORALOS_CLI_VERSION=%q\n' "$NORALOS_CLI_VERSION"
  } >"$SMOKE_METADATA_FILE"
}
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      ...
      'timeout 20s npx --yes "noralos@${NORALOS_CLI_VERSION}" auth bootstrap-ceo --data-dir "$NORALOS_HOME" --base-url "$NORALOS_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$NORALOS_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$NORALOS_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$NORALOS_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$NORALOS_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$NORALOS_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$NORALOS_PUBLIC_URL/api/companies")"
...
docker build \
  --build-arg NORALOS_CLI_VERSION="$NORALOS_CLI_VERSION" \
...
echo "    Public URL: $NORALOS_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Detached mode: $SMOKE_DETACH"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $NORALOS_DEPLOYMENT_MODE/$NORALOS_DEPLOYMENT_EXPOSURE"
...
if ! wait_for_http "$NORALOS_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $NORALOS_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$NORALOS_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
...
if [[ "$SMOKE_DETACH" == "true" ]]; then
  ...
  echo "    Smoke base URL: $NORALOS_PUBLIC_URL"
  echo "    Smoke admin credentials: $SMOKE_ADMIN_EMAIL / $SMOKE_ADMIN_PASSWORD"
