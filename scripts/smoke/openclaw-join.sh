...
COMPANY_ID="${COMPANY_ID:-${NORALOS_COMPANY_ID:-}}"
...
Provide one of:
  NORALOS_AUTH_HEADER=\"Bearer <board-token>\"
  NORALOS_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "NoralOS OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
