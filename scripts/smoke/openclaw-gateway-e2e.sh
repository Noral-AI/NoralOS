...
NORALOS_API_URL="${NORALOS_API_URL:-http://127.0.0.1:3100}"
API_BASE="${NORALOS_API_URL%/}/api"
...
NORALOS_API_URL_FOR_OPENCLAW="${NORALOS_API_URL_FOR_OPENCLAW:-http://host.docker.internal:3100}"
...
AUTH_HEADERS=()
if [[ -n "${NORALOS_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=( -H "Authorization: ${NORALOS_AUTH_HEADER}" )
fi
if [[ -n "${NORALOS_COOKIE:-}" ]]; then
  AUTH_HEADERS+=( -H "Cookie: ${NORALOS_COOKIE}" )
  NORALOS_BROWSER_ORIGIN="${NORALOS_BROWSER_ORIGIN:-${NORALOS_API_URL%/}}"
  AUTH_HEADERS+=( -H "Origin: ${NORALOS_BROWSER_ORIGIN}" -H "Referer: ${NORALOS_BROWSER_ORIGIN}/" )
fi
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board auth required. Set NORALOS_COOKIE or NORALOS_AUTH_HEADER."
  fi
...
create_and_approve_gateway_join() {
  ...
  join_payload="$(jq -nc \
    ...
    --arg noralosApiUrl "$NORALOS_API_URL_FOR_OPENCLAW" \
...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set NORALOS_API_KEY=${AGENT_API_KEY} in your run context before running NoralOS heartbeat steps."
...
main() {
  ...
  wait_http_ready "${NORALOS_API_URL%/}/api/health" 15 || fail "NoralOS API health endpoint not reachable"
