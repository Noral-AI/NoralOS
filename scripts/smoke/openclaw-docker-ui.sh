...
NORALOS_HOST_PORT="${NORALOS_HOST_PORT:-3100}"
NORALOS_HOST_FROM_CONTAINER="${NORALOS_HOST_FROM_CONTAINER:-host.docker.internal}"
...
detect_paperclip_base_url() {
  ...
  for candidate in "$NORALOS_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${NORALOS_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${NORALOS_HOST_PORT}"
      return 0
    fi
  done
  return 1
