...
- **Standalone MCP server** — New `@noralos/mcp-server` package exposing the Paperclip API as an MCP tool server, including approval creation. ([#2435](https://github.com/paperclipai/paperclip/pull/2435))
...
- **Agent auth JWT** — Fixed agent auth to fall back to `BETTER_AUTH_SECRET` when `NORALOS_AGENT_JWT_SECRET` is absent. ([#2866](https://github.com/paperclipai/paperclip/pull/2866), @ergonaworks)
