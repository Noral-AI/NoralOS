...
All migrations are additive (new tables and columns) — no existing data is modified. Standard `noralos` startup will apply them automatically.

If you use the `.env` file, note that the server now falls back to loading `.env` from the current working directory when `.noralos/.env` is not found.
