...
Add first-class **documents** to NoralOS as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by NoralOS workflow/docs.
...
Update the NoralOS agent workflow so planning no longer edits the issue description.
