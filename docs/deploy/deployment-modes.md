...
NoralOS supports two runtime modes with different security profiles. Reachability is configured separately with `bind`.
...
```sh
pnpm noralos allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, NoralOS emits a one-time claim URL at startup:
...
```sh
NORALOS_DEPLOYMENT_MODE=authenticated NORALOS_BIND=lan pnpm noralos run
```
