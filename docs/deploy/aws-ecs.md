...
summary: Deploy NoralOS to AWS using ECS Fargate, RDS Postgres, and EFS
---

Deploy NoralOS to AWS with ECS Fargate (compute), RDS Postgres 17 (database), and EFS (persistent storage). This guide uses the AWS CLI and produces a single-task ECS service behind an ALB with HTTPS.
...
- The NoralOS repo cloned locally
...
export NORALOS_DOMAIN=paperclip.example.com   # your domain
...
aws rds create-db-subnet-group \
  ...
  --db-subnet-group-description "NoralOS RDS subnets" \
...
- Create a CNAME or ALIAS record for `$NORALOS_DOMAIN` -> `$ALB_DNS`
...
#   { "name": "NORALOS_AUTH_DISABLE_SIGN_UP", "value": "true" }
