...
summary: Build, package, and distribute adapters as plugins without modifying NoralOS source
---

NoralOS supports external adapter plugins that can be installed from npm packages or local directories. External adapters work exactly like built-in adapters — they execute agents, parse output, and render transcripts — but they live in their own package and don't require changes to NoralOS's source code.
...
| Distribution | Ships with NoralOS | Published to npm or linked via `file:` |
| Updates | Requires NoralOS release | Independent versioning |
...
Local adapters are symlinked into NoralOS's adapter directory. Changes to the source are picked up on server restart.
...
Other NoralOS users can then install your adapter by package name from the UI or API.
