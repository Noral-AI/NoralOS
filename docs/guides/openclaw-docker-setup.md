...
How to get OpenClaw running in a Docker container for local development and testing the NoralOS OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

NoralOS includes an end-to-end join smoke harness:
...
- probes and prints a NoralOS host URL that is reachable from inside OpenClaw Docker
...
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host NoralOS server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed NoralOS URL (typically `http://host.docker.internal:3100`).
