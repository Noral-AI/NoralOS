...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-NoralOS-Signature` + `X-NoralOS-Timestamp` header pair matching the trigger's signing mode.
