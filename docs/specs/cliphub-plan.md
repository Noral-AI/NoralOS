# ClipHub: Marketplace for NoralOS Team Configurations
...
> The "app store" for whole-company AI teams — pre-built NoralOS configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for NoralOS-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | NoralOS company (agents, projects, governance) |
...
A complete NoralOS company configuration:
...
Single-agent configurations designed to plug into a NoralOS org:
...
Portable skill files that any NoralOS agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with NoralOS's skill loading system
...
interface Listing {
  ...
  paperclipVersionMin: string;     // Minimum NoralOS version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target NoralOS company
...
| `POST` | `/api/listings/:id/install` | Install to NoralOS company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target NoralOS company (or create new)
  → ClipHub API calls NoralOS API to:
  ...
  → Redirect to NoralOS dashboard with new team running
```
...
### 5.3 Creator: Export from NoralOS → Publish

```
Running NoralOS company → "Export as Blueprint" (CLI or UI)
  → NoralOS exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (NoralOS brand blue/purple)
...
| Install flow | One-click deploy to NoralOS company |
...
{
  "targetCompanyId": "uuid",         // Existing NoralOS company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `noralos-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as NoralOS
- **Backend**: Node.js API (or extend NoralOS server)
- **Database**: Postgres (can share NoralOS's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with NoralOS auth (or OAuth2)

### 9.2 Integration with NoralOS

ClipHub can be:
- **Option A**: A separate app that calls NoralOS's API to install blueprints
- **Option B**: A built-in section of the NoralOS UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing NoralOS UI and API.
...
- [ ] Install flow: blueprint → NoralOS company
...
- [ ] "Export from NoralOS" CLI command
