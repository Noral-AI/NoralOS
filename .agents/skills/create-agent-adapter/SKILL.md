...
description: >
  Technical guide for creating a new NoralOS agent adapter. Use when building
...
# Creating a NoralOS Agent Adapter

An adapter bridges NoralOS's orchestration layer to a specific AI agent runtime (Claude Code, Codex CLI, a custom process, an HTTP endpoint, etc.). Each adapter is a self-contained package that provides implementations for **three consumers**: the server, the UI, and the CLI.
...
interface AdapterExecutionResult {
  ...
  clearSession?: boolean;         // true = tell NoralOS to forget the stored session
}
...
The `agentConfigurationDoc` is read by LLM agents (including NoralOS agents that create other agents). Write it as **routing logic**, not marketing copy. Include concrete "use when" and "don't use when" guidance so an LLM can decide whether this adapter is appropriate for a given task.
...
const myAgentAdapter: ServerAdapterModule = {
  ...
  models: myModels,
  supportsLocalAgentJwt: true,  // true if agent can use NoralOS API
...
- **Unknown session retry**: if resume fails with a "session not found" error, retry with a fresh session and return `clearSession: true` so NoralOS wipes the stale session
...
Codex has a global personal skills directory (`$CODEX_HOME/skills` or `~/.codex/skills`). The adapter symlinks NoralOS skills there if they don't already exist. This is acceptable because it's the agent tool's own config directory, not the user's project.
...
Adapters sit at the boundary between NoralOS's orchestration layer and arbitrary agent execution. This is a high-risk surface.
...
- Prefer minimal allowlists over open internet access. An agent that only needs to call the NoralOS API and GitHub should not have access to arbitrary hosts.
