...
import { ApiConnectionError, ApiRequestError, NoralosApiClient } from "../client/http.js";

describe("NoralosApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new NoralosApiClient({
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new NoralosApiClient({ apiBase: "http://localhost:3100" });
  ...
  it("throws ApiConnectionError with recovery guidance when fetch fails", async () => {
    ...
    await expect(client.post("/api/companies/import/preview", {})).rejects.toThrow(
      /Could not reach the NoralOS API\./,
    );
    await expect(client.post("/api/companies/import/preview", {})).rejects.toThrow(
      /curl http:\/\/localhost:3100\/api\/health/,
    );
    await expect(client.post("/api/companies/import/preview", {})).rejects.toThrow(
      /pnpm dev|pnpm noralos run/,
    );
  });
