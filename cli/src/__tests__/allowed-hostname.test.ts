...
function writeBaseConfig(configPath: string) {
  const base: NoralosConfig = {
    ...
    database: {
      ...
      embeddedPostgresDataDir: "/tmp/noralos-db",
      embeddedPostgresPort: 54329,
      backup: {
        enabled: true,
        intervalMinutes: 60,
        retentionDays: 30,
        dir: "/tmp/noralos-backups",
      },
    },
    logging: {
      mode: "file",
      logDir: "/tmp/noralos-logs",
    },
    server: {
    ...
    storage: {
      provider: "local_disk",
      localDisk: { baseDir: "/tmp/noralos-storage" },
      s3: {
        bucket: "paperclip",
        region: "us-east-1",
        prefix: "",
    ...
    secrets: {
      ...
      strictMode: false,
      localEncrypted: { keyFilePath: "/tmp/noralos-secrets/master.key" },
    },
  };
...
describe("allowed-hostname command", () => {
  it("adds and normalizes hostnames", async () => {
    ...
    const raw = JSON.parse(fs.readFileSync(configPath, "utf-8")) as NoralosConfig;
