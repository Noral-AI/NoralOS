...
describe("applyDataDirOverride", () => {
  ...
  it("sets NORALOS_HOME and isolated default config/context paths", () => {
    ...
    expect(process.env.NORALOS_HOME).toBe(expectedHome);
    expect(process.env.NORALOS_CONFIG).toBe(
      path.resolve(expectedHome, "instances", "default", "config.json"),
    );
    expect(process.env.NORALOS_CONTEXT).toBe(path.resolve(expectedHome, "context.json"));
    expect(process.env.NORALOS_INSTANCE_ID).toBe("default");
  });
  ...
  it("does not override explicit config/context settings", () => {
    process.env.NORALOS_CONFIG = "/env/config.json";
    ...
    expect(process.env.NORALOS_CONFIG).toBe("/env/config.json");
    expect(process.env.NORALOS_CONTEXT).toBe("/env/context.json");
  });
