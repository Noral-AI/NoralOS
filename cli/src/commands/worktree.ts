...
import { ensureAgentJwtSecret, loadNoralosEnvFile, mergeNoralosEnvEntries, readNoralosEnvEntries, resolveNoralosEnvFile } from "../config/env.js";
...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.NORALOS_CONFIG;
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.NORALOS_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}

function resolveWorktreeStartPoint(explicit?: string): string | undefined {
  return explicit ?? nonEmpty(process.env.NORALOS_WORKTREE_START_POINT) ?? undefined;
}
...
export function resolveWorktreeReseedTargetPaths(input: {
  ...
  const envEntries = readNoralosEnvEntries(resolveNoralosEnvFile(input.configPath));
  const homeDir = nonEmpty(envEntries.NORALOS_HOME);
  const instanceId = nonEmpty(envEntries.NORALOS_INSTANCE_ID);

  if (!homeDir || !instanceId) {
    throw new Error(
      `Target config ${input.configPath} does not look like a worktree-local NoralOS instance. Expected NORALOS_HOME and NORALOS_INSTANCE_ID in the adjacent .env.`,
    );
  }
...
export function copySeededSecretsKey(input: {
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.NORALOS_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.NORALOS_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.NORALOS_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.NORALOS_SECRETS_MASTER_KEY_FILE) : null);
...
async function seedWorktreeDatabase(input: {
  ...
  const sourceEnvFile = resolveNoralosEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readNoralosEnvEntries(sourceEnvFile);
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  const sourceEnvEntries = readNoralosEnvEntries(resolveNoralosEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.NORALOS_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.NORALOS_AGENT_JWT_SECRET);
  mergeNoralosEnvEntries(
    {
      ...buildWorktreeEnvEntries(paths, branding),
      ...(existingAgentJwtSecret ? { NORALOS_AGENT_JWT_SECRET: existingAgentJwtSecret } : {}),
    },
    paths.envPath,
  );
  ensureAgentJwtSecret(paths.configPath);
  loadNoralosEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run NoralOS inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolveNoralosEnvFile(configPath);
  const envEntries = readNoralosEnvEntries(envPath);
  const out = {
    NORALOS_CONFIG: configPath,
    ...(envEntries.NORALOS_HOME ? { NORALOS_HOME: envEntries.NORALOS_HOME } : {}),
    ...(envEntries.NORALOS_INSTANCE_ID ? { NORALOS_INSTANCE_ID: envEntries.NORALOS_INSTANCE_ID } : {}),
    ...(envEntries.NORALOS_CONTEXT ? { NORALOS_CONTEXT: envEntries.NORALOS_CONTEXT } : {}),
    ...envEntries,
  };
...
async function openConfiguredDb(configPath: string): Promise<OpenDbHandle> {
  ...
  const envEntries = readNoralosEnvEntries(resolveNoralosEnvFile(configPath));
    ...
    if (migrationState.status !== "upToDate") {
      const pending =
        migrationState.reason === "pending-migrations"
          ...
          : "";
      throw new Error(
        `Database for ${configPath} is not up to date.${pending} Run \`pnpm db:migrate\` (or start NoralOS once) before using worktree merge history.`,
      );
    }
...
async function promptForSourceEndpoint(excludeWorktreePath?: string): Promise<ResolvedWorktreeEndpoint> {
  ...
  if (choices.length === 0) {
    throw new Error("No NoralOS worktrees were found. Run `paperclipai worktree:list` to inspect the repo worktrees.");
  }
...
export async function worktreeMergeHistoryCommand(sourceArg: string | undefined, opts: WorktreeMergeHistoryOptions): Promise<void> {
  ...
  if (path.resolve(sourceEndpoint.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target NoralOS configs are the same. Choose different --from/--to worktrees.");
  }
...
async function runWorktreeReseed(opts: WorktreeReseedOptions): Promise<void> {
  ...
  if (path.resolve(source.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target NoralOS configs are the same. Choose different --from/--to values.");
  }
  ...
  if (runningTargetPid && !opts.allowLiveTarget) {
    throw new Error(
      `Target worktree database appears to be running (pid ${runningTargetPid}). Stop NoralOS in ${targetEndpoint.rootPath} before reseeding, or re-run with --allow-live-target if you want to override this guard.`,
    );
  }
  ...
  const confirmed = opts.yes
    ? true
    : await p.confirm({
      message: `Overwrite the isolated NoralOS DB for ${targetEndpoint.label} from ${source.label} using ${seedMode} seed mode?`,
      initialValue: false,
    });
  if (p.isCancel(confirmed) || !confirmed) {
...
export async function worktreeRepairCommand(opts: WorktreeRepairOptions): Promise<void> {
  ...
  if (path.resolve(source.configPath) === path.resolve(target.configPath)) {
    throw new Error("Source and target NoralOS configs are the same. Use --from-config/--from-instance to point repair at a different source.");
  }

  const targetConfig = existsSync(target.configPath) ? readConfig(target.configPath) : null;
  const targetEnvEntries = readNoralosEnvEntries(resolveNoralosEnvFile(target.configPath));
  const targetHasWorktreeEnv = Boolean(
    nonEmpty(targetEnvEntries.NORALOS_HOME) && nonEmpty(targetEnvEntries.NORALOS_INSTANCE_ID),
  );
  ...
  if (runningTargetPid && !opts.allowLiveTarget) {
    throw new Error(
      `Target worktree database appears to be running (pid ${runningTargetPid}). Stop NoralOS in ${target.rootPath} before repairing, or re-run with --allow-live-target if you want to override this guard.`,
    );
  }
  if (runningTargetPid && opts.allowLiveTarget) {
    p.log.warning(`Proceeding even though the target embedded PostgreSQL appears to be running (pid ${runningTargetPid}).`);
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local NoralOS instance helpers");
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local NoralOS instance")
  ...
  program
    ...
    .description("List git worktrees visible from this repo and whether they look like NoralOS worktrees")
  ...
  worktree
    .command("reseed")
    .description("Re-seed an existing worktree-local instance from another NoralOS instance or worktree")
    .option("--from <worktree>", "Source worktree path, directory name, branch name, or current")
    .option("--to <worktree>", "Target worktree path, directory name, branch name, or current (defaults to current)")
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source NORALOS_HOME used when deriving the source config")
