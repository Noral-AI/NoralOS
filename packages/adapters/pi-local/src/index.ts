...
Use when:
- You want NoralOS to run Pi (the AI coding agent) locally as the agent runtime
...
- NoralOS requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/noraloss/ and resumed with --session.
