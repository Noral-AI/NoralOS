...
## Scope
This plan is now **gateway-only**. NoralOS supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses NoralOS skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a NoralOS task.
...
```bash
pnpm --filter @noralos/server test:run -- openclaw-gateway
pnpm --filter @noralos/server typecheck
pnpm --filter @noralos/ui typecheck
