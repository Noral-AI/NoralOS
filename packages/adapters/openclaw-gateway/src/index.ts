...
Use when:
- You want NoralOS to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the NoralOS server.
...
- noralosApiUrl (string, optional): absolute NoralOS base URL advertised in wake text
- claimedApiKeyPath (string, optional): path to the claimed API key JSON file read by the agent at wake time (default ~/.openclaw/workspace/noralos-claimed-api-key.json)
...
- paperclip (object): standardized NoralOS context added to every gateway agent request
- paperclip.workspace (object, optional): resolved execution workspace for this run
- paperclip.workspaces (array, optional): additional workspace hints NoralOS exposed to the run
