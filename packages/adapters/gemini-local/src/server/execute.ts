...
function renderNoralosEnvNote(env: Record<string, string>): string {
  ...
  return [
    "NoralOS runtime note:",
    `The following NORALOS_* environment variables are available in this run: ${noralosKeys.join(", ")}`,
...
function renderApiAccessNote(env: Record<string, string>): string {
  if (!hasNonEmptyEnvValue(env, "NORALOS_API_URL") || !hasNonEmptyEnvValue(env, "NORALOS_API_KEY")) return "";
  return [
    "NoralOS API access note:",
    "Use run_shell_command with curl to make NoralOS API requests.",
    "GET example:",
    `  run_shell_command({ command: "curl -s -H \\"Authorization: Bearer $NORALOS_API_KEY\\" \\"$NORALOS_API_URL/api/agents/me\\"" })`,
    "POST/PATCH example:",
    `  run_shell_command({ command: "curl -s -X POST -H \\"Authorization: Bearer $NORALOS_API_KEY\\" -H 'Content-Type: application/json' -H \\"X-NoralOS-Run-Id: $NORALOS_RUN_ID\\" -d '{...}' \\"$NORALOS_API_URL/api/issues/{id}/checkout\\"" })`,
    "",
    "",
  ].join("\n");
}
...
/**
 * Inject NoralOS skills directly into `~/.gemini/skills/` via symlinks.
...
async function ensureGeminiSkillsInjected(
  ...
  for (const entry of selectedEntries) {
    ...
    try {
      const result = await ensureNoralosSkillSymlink(entry.source, target);
...
async function buildGeminiSkillsDir(
  ...
  const desiredNames = new Set(resolveNoralosDesiredSkillNames(config, availableEntries));
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const geminiSkillEntries = await readNoralosRuntimeSkillEntries(config, __moduleDir);
  const desiredGeminiSkillNames = resolveNoralosDesiredSkillNames(config, geminiSkillEntries);
  ...
  const noralosEnvNote = renderNoralosEnvNote(env);
  ...
  const promptMetrics = {
    ...
    runtimeNoteChars: noralosEnvNote.length + apiAccessNote.length,
