 ...
 *
 * These components are exported from `@noralos/plugin-sdk/ui` and are
 ...
 *
 * @see PLUGIN_SPEC.md §19.6 — Shared Components In `@noralos/plugin-sdk/ui`
 */
...
function createSdkUiComponent<TProps>(name: string): React.ComponentType<TProps> {
  return function NoralosSdkUiComponent(props: TProps) {
 ...
 * @example
 * ```tsx
 * import { FileTree, type FileTreeNode } from "@noralos/plugin-sdk/ui";
 *
