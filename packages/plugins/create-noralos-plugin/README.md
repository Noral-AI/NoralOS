# @noralos/create-noralos-plugin

Scaffolding tool for creating new NoralOS plugins.

```bash
npx @noralos/create-noralos-plugin my-plugin
```

Or with options:

```bash
npx @noralos/create-noralos-plugin @acme/my-plugin \
  --template connector \
  --category connector \
...
Inside this repo, the generated package uses `@noralos/plugin-sdk` via `workspace:*`.
