...
Use this file as the kickoff context for the next Claude Code session against this fork. Read `NORALOS_AUDIT.md` for the full Phase 0 recon report; this file is the action-oriented pickup brief.
...
1. **Resolve Open Issues #1, #2, #3, #5 from `NORALOS_AUDIT.md`** — clarify or revise the migration plan's assumptions before code work. Specifically:
...
3. **Update `NORALOS_MIGRATION.md` Phase 3 narrative** — the `<TBD>` tags are filled in by this audit, but the decision tree ("PARA sufficient or plugin?") is moot since there's no built-in PARA layer. Rewrite the Phase 3 decision section to reflect "use para-memory-files skill vs build a memory plugin."
...
- **The `LICENSE` file** — under MIT, the upstream copyright (`Copyright (c) 2025 Paperclip AI`) **must be preserved** even during the rebrand. This is a legal hard floor. The maximum rebrand target is "no NoralOS references except the LICENSE copyright line and minimal NOTICE attribution to upstream `paperclipai/paperclip`."
...
| 2. Skills + scripts + tests | `skills/noralos*/` (rename + content), `.agents/skills/*/SKILL.md` content, `scripts/noralos-*`, `ui/storybook/fixtures/noralosData.ts`, `ui/src/hooks/useNoralOSIssueRuntime.ts`, `server/src/__tests__/noralos-*.test.ts` | ~150 |
| 3. Env vars | `NORALOS_HOME` → `NORALOS_HOME` (or chosen name), `NORALOS_CONFIG`, `NORALOS_INSTANCE_ID`, `NORALOS_DEPLOYMENT_MODE`, `NORALOS_DEPLOYMENT_EXPOSURE`, plus all `.env.example`, Dockerfile, docker-compose, ecs-task-definition.json, quadlet | ~30 |
| 4. Filesystem paths | `/noralos` → `/noralos`. Dockerfile `VOLUME`, `WORKDIR`, scripts, runbooks, docs | ~50 |
...
3. **Rebrand: target name and namespace?**
   ...
   - Database default user/db name in `.env.example`: `noralos:noralos@localhost:5432/noralos` or keep `noralos` as a generic placeholder for the dev-only example?
...
- `NORALOS_AUDIT.md` — main audit report (subsystems, deltas, open issues, limitations, recommendations)
- `NORALOS_TREE.md` — depth-3 repo tree (substituted for tree(1) since not installed locally)
- `NORALOS_NEXT_SESSION.md` — this file
- `NORALOS_MIGRATION.md` — copy of Quentin's local `/Users/quentin/Documents/NORALAI/NORALOS/noralOS/NORALOS_MIGRATION.md` with all 12 `<TBD: ...>` tags filled in with real paths from this audit. Decision narrative for Phase 3 not rewritten — see "Safe to edit next" item #3.
